  ...
  <div class="novel" ref="novelRef">
    <div class="headBtn jb ac" ref="headBtnRef">
      <t-space>
        ...
        <t-button @click="quickPasteShow = true">
          <template #icon>
            <t-icon name="paste" />
          </template>
          {{ $t("workbench.novel.quickPaste") }}
        </t-button>
    ...
    <!-- 快捷粘贴对话框 -->
    <t-dialog
      v-model:visible="quickPasteShow"
      :header="$t('workbench.novel.quickPasteTitle')"
      width="50%"
      placement="center"
      :confirm-btn="{ content: $t('workbench.novel.import.msg.saveSuccess').replace('成功',''), theme: 'primary', loading: quickPasteSaving }"
      @confirm="handleQuickPaste">
      <p style="margin-bottom: 12px; color: var(--td-text-color-secondary)">{{ $t("workbench.novel.quickPasteHint") }}</p>
      <t-textarea v-model="quickPasteContent" :autosize="{ minRows: 15, maxRows: 25 }" :placeholder="$t('workbench.novel.quickPasteHint')" />
      <div style="margin-top: 8px; color: var(--td-text-color-placeholder); font-size: 12px">{{ quickPasteContent.length }} {{ $t("workbench.novel.import.chars") }}</div>
    </t-dialog>
  </div>
</template>
...
// 快捷粘贴
const quickPasteShow = ref(false);
const quickPasteContent = ref("");
const quickPasteSaving = ref(false);

async function handleQuickPaste() {
  if (!quickPasteContent.value.trim()) {
    window.$message.warning($t("workbench.novel.quickPasteEmpty"));
    return;
  }
  quickPasteSaving.value = true;
  try {
    await axios.post("/novel/addNovel", {
      projectId: project.value?.id,
      data: [{ index: 1, reel: "正文卷", chapter: "第1章", chapterData: quickPasteContent.value.trim() }],
    });
    window.$message.success($t("workbench.novel.quickPasteSaved"));
    quickPasteShow.value = false;
    quickPasteContent.value = "";
    getNovel();
  } catch (e) {
    window.$message.error((e as Error).message);
  } finally {
    quickPasteSaving.value = false;
  }
}
</script>
