...
const { connected, messages, status, currentScriptId: episodesId, loadingHistory } = storeToRefs(storyboardAgentStore());
...
function handleSend(text: string) {
  storyboardAgentStore().chat(text);
  inputValue.value = "";
}
function handleStop() {
  storyboardAgentStore().stopGenerate();
}
...
const handleActions = {
  suggestion: (data?: any) => {
    storyboardAgentStore().chat(data?.content?.prompt);
  },
};
...
function handleClearMemory(type: "message" | "summary" | "all") {
  const dialog = DialogPlugin.confirm({
    ...
    theme: "warning",
    onConfirm: async () => {
      await axios.post(`/agents/clearMemory`, { projectId: project.value?.id, agentType: "storyboardAgent", episodesId: episodesId.value, type });
      window.$message.success($t("workbench.production.chatBox.memoryCleared", { type: memoryTypeLabel[type] }));
      dialog.destroy();
      storyboardAgentStore().getHistory();
    },
  });
}
