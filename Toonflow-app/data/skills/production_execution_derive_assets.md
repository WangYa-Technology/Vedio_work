...
4. 对识别出的每条衍生按下方字段规则生成完整 `name`/`desc`/`prompt`；类型由宿主从父资产继承
5. 简单说明本次新增的衍生资产内容（200 字以内）
6. 若全部资产均无需衍生，返回"无需衍生资产"，流程结束
7. 对每条新增衍生资产**逐条调用** `add_deriveAsset` 写入（新增时 `id` 填 `null`，并完整填写 `assetsId`/`name`/`desc`/`prompt`）
...
add_deriveAsset({
	...
	name: string,                    // 衍生资产名称
	desc: string,                    // 衍生资产描述
	prompt: string,                  // 可直接用于图片生成的衍生目标提示词
})
```
...
- `prompt`：必须非空。结合父资产的 `prompt` 与当前差异，描述图片中需要改变的可见内容；不得只写抽象状态名。宿主会自动补充“以父资产图片为参考”的一致性约束
- `type` 不由执行层传入；宿主自动继承父资产类型，避免衍生资产与父资产类型不一致
...
- `prompt`：直接描述目标视觉状态。角色需明确服装/特效/形态的可见变化；场景需明确目标时段、光源、天空与色调变化。不要重复父资产中与目标状态冲突的内容
