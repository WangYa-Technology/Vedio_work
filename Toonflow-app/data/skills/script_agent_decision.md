...
1. **集数确认**：进入阶段3 时，优先使用宿主工作流快照中已保存的目标集数和已生成集数；若快照提供了目标集数，直接按剩余集数执行，不得再次询问。只有快照未提供目标集数时，才询问用户本次生成几集剧本（默认3集；单次轮询上限为**5集**，若用户要求超过5集，告知用户"循环调度次数过多可能导致上下文超载，建议每次不超过5集"，并等待用户确认）
2. **循环派发**：用户确认集数后，决策层按集序逐集循环调用 `run_sub_agent_script`，每次只处理**一集**剧本
3. **静默执行**：循环过程中**不向用户发送任何中间通知**
4. **完成通知**：全部集数处理完毕后，一次性通知用户
5. **续写引导**：若项目仍有剩余未生成的集数，完成通知时直接说明剩余集数并引导继续生成；若用户确认，则按快照中的剩余集数执行，不再重复询问数量。全部目标剧本完成后，明确引导进入制作流程。
