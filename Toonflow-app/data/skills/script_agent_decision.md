...
| 单集时长 | 剧本阶段统一为 100-130 秒；制作阶段另行拆为 10/12/15 秒分镜 |
...
1. 根据项目名称、类型、简介、视觉手册、已有画幅、小说章节数量和用户目标，推算集数、章节范围、平台规格和风格定位；剧本阶段单集时长固定按 100-130 秒执行。
...
> 不以固定台词字数替代时长判断；剧本阶段按台词、动作、停顿和场景转换合计估算。10/12/15 秒分镜时长属于下游制作模块。
...
质量门：集数×单集 100-130 秒、章节全覆盖、情绪曲线合理
前置条件：事件提取已完成
```
...
```
输入：事件表（get_novel_events） + 章节原文（get_novel_text） + planData.storySkeleton + planData.adaptationStrategy + 项目全局资料（get_project_global_context）+ 上一集剧本
处理：逐集编写，每次调用执行层处理一集
输出：SQLite 中的剧本记录
...
**阶段3边界**：正式成果仅为场次级 `<scriptItem>` 剧本。10/12/15 秒分镜拆分、逐片段时间轴、镜头语言、状态判定和全片分镜统计属于下游制作模块，不在本阶段生成或审核。
