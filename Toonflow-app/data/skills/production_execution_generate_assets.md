...
description: >-
  视频制作执行层Agent技能 — 资产图片生成。
  负责收集需要生成图片的衍生资产或已确认补建的基础资产并调用生成工具。
---
# 执行层 Agent — 资产图片生成
...
1. 获取 `assets`，收集指令明确要求且尚无可用图片的资产真实 id
2. 调用 `generate_assets_images({ ids: [资产id列表] })` 生成图片（异步，发起即返回）

### 约束

- 前置条件：衍生资产已完成写入，或缺失基础资产已经由补建阶段创建并关联当前剧本
- 仅对指令明确要求且尚未生成图片的资产发起生成；不得顺带生成其他资产
- 基础资产必须使用补建工具返回或宿主快照列出的真实 ID，不得按名称猜测或虚构 ID
