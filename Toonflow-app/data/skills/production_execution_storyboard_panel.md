...
| 参数 | 类型 | 说明 |
|------|------|------|
| `videoDesc` | `string` | 画面描述、地点、时段、内外、空间层级、关联资产名称、时长、景别、景别说明、摄影角度、运镜、台词、音效、关联资产ID（**故事板辅助多参模式**为固定文本） |
...
2. **该场空间信息 + 该组分镜行原文**：先完整保留场头下方的地点、时段、内外、空间层级，再完整保留该组全部分镜行的原始文字（序号、画面描述、时长、景别、景别说明、摄影角度、运镜、台词、音效各列内容一字不改）。
...
**第 2 步 · 为每组生成故事板图提示词**
每组只生成一条 `prompt`，以该组第一行的画面状态作为故事板图瞬间，必须保留主体、地点、时段、内外、空间层级、景别、景别说明所约束的主体占比/相对位置、角色位置/朝向和关键道具；按资产绑定顺序使用 `@图N` 标注对应参考图。提示词不能为空，也不得把整组动作写成连续视频脚本。
...
根据 `storyboardTable` 对应场头的空间信息与对应行的完整分镜数据（画面描述、地点、时段、内外、空间层级、关联资产名称、时长、景别、景别说明、摄影角度、运镜、台词、音效、关联资产ID），整合为一段结构化视频描述文本，填入 `videoDesc` 字段。**禁止包含任何光影/色温/明暗/色调描述**。

**第 6 步 · 生成提示词（prompt）并忠实性校验**
逐行读取 `storyboardTable` 对应场头的「地点」「时段」「内外」「空间层级」与对应行的「画面描述」「景别」「景别说明」「摄影角度」「运镜」字段，严格按已加载技法中的「分镜表内容忠实性原则」和「解析映射规则」将各字段映射为提示词各段落。**提示词正文不得包含光影/色温/明暗/色调描述**。**生成每条提示词后须立即逐字段比对分镜表原始内容**，确认：
...
- **前置条件**：分镜表已构建完成且用户已确认
- **videoDesc 必填**：每条分镜的 `videoDesc` 必须根据 `storyboardTable` 对应场头和对应行生成，包含画面描述、地点、时段、内外、空间层级、关联资产名称、时长、景别、景别说明、摄影角度、运镜、台词、音效、关联资产ID 等完整信息（**故事板辅助多参模式例外**——`videoDesc` 为固定文本 `参考故事板内容进行视频生成`，画面信息由故事板图承载）
