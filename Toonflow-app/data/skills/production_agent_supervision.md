...
7. **缺资产必须进入补充闭环**：剧本或分镜中出现但 assets 中无对应基础资产的角色/道具/场景，必须作为阻塞问题报告，给出名称、类型和“由生产 Agent 补建基础资产并生成图片”的 A 方案；可同时提供“改为不可辨识背景元素”的 B 方案。不得虚构资产 ID。资产补齐并生成图片后，再审核其引用/关联/衍生覆盖
...
- 画面中可辨识的角色必须引用对应资产 ID（含背影/肢体局部/虚化身影）；assets 中无对应资产时，监督层必须报告缺少资产并阻止进入后续生成，直到资产库补充完成
...
**不审核**：
- 不把已删除的额外字段（景别说明、摄影角度、空间信息）作为审核项；标准镜表只检查七列核心字段。
...
| 可见角色关联完整 | 严重 | 画面中可辨识的角色（含背影/肢体局部/虚焦剪影）必须出现在该片段引用资产名称/引用资产ID及场头参演角色中；assets 中不存在时列为“缺少资产”，建议由生产 Agent 补建并生成图片 | R1 |
| 场景资产关联 | 严重 | 每个片段引用资产ID须含所处场景的 scene 资产 ID（存在匹配衍生时用衍生 ID）；assets 中不存在时列为“缺少资产”，建议由生产 Agent 补建并生成图片 | R1 |
...
5. **缺少资产必须报告**：画面描述提及但 assets 中无对应资产的角色，必须报告资产名称和类型，并建议由生产 Agent 补建后生成图片；不得虚构资产 ID。

不通过示例：assets 中已有"凌玄"和"青云令"，画面描述写"凌玄手持青云令"，但片段 引用资产ID 只有凌玄，遗漏青云令。
缺少资产示例：assets 中无"何鸿燊"资产，画面描述出现"何鸿燊出镜+台词"——报告“缺少角色资产：何鸿燊”，A 方案为由生产 Agent 补建角色并生成图片。

#### 场景资产关联（→ R1）

1. 从场头读取 场景名，定位该场对应的 scene 资产
2. **缺少场景资产必须报告**：assets 中无匹配该场景的 scene 资产时，报告“缺少场景资产”，建议由生产 Agent 补建场景并生成图片。
