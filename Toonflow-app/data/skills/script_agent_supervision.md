...
1. **8大核心要点覆盖**：策略是否体现——强画面感、原文台词保留与密度控制、节奏极致快、只沿主线、降低理解成本、情绪大于一切、开篇给足期待感、展示不要告诉（动作>台词）
...
6. **短剧语言适配**：原文称谓和台词不因风格偏好被改写；仅当用户授权新增/改写时，新增内容才检查称谓与口语化
...
3. 调用 `get_novel_events` 与 `get_novel_text` 读取受审集覆盖的事件和章节原文，核对原文事实与原文台词。
4. 按受审集数、人物和地点调用 `get_project_global_context`，校验前后集连续性、人物知识边界、秘密揭示时机、地点/组织/时间线规则。
5. 不得用记忆补齐缺失设定；无法确认的内容标记为「待作者补充」，不能直接判定为漏洞。

### 证据优先级与冲突处理

当前用户明确指令 > 已确认改编设定 > 原著事实与原文台词 > 项目全局资料 > 骨架/策略中的派生描述 > 模型推断。

- 全局资料标记为 `proposed` 或 `unresolved` 时，只能列为待确认，不能据此判定剧本错误或替作者定案。
- 全局资料与原文、已确认骨架或用户指令冲突时，报告具体冲突与位置，不静默选择低优先级资料覆盖。
- 未来剧情、秘密和结局只能在允许揭示的集数进入角色知识或观众信息，不得因审核读取到全局资料而要求提前泄露。
...
12. **原文台词**：被采用场景的原文台词是否逐字保留；用户未明确授权时是否擅自新增、改写或删除人物对白。
13. **单集时长**：台词、动作、停顿和场景转换合计是否在 100-130 秒内；只做剧本级总时长判断。
14. **模块边界**：剧本是否越权包含景别、机位、运镜、10/12/15 秒分镜、逐片段时间轴、分镜状态判定或全片统计。
...
- 本阶段审核对象是场次级剧本，不是制作侧分镜表。
- 不输出逐片段时间轴、10/12/15 秒拆分、分镜状态判定或全片分镜统计。
- 发现单集总时长超过 130 秒时，只指出重复内容、低密度段落和自然拆场/跨集承接位置；不得自动生成分镜并复验。
...
2. 从【项目配置】读取：集数、章节范围；单集按 100-130 秒校验
...
| 分集与时长 | 分集数恰好等于【项目配置】集数；每集按 100-130 秒容量规划 | 中等 |
...
2. 从【项目配置】读取平台规格；单集按 100-130 秒容量约束审核
...
| 8大要点覆盖 | 策略体现强画面感、原文台词保留与密度控制、节奏极致快、只沿主线、降低理解成本、情绪大于一切、开篇给足期待感、展示不要告诉（→ Skills 二-1） | 中等 |
...
| 语言适配 | 未授权时原文称谓和台词保持不变；授权新增/改写时才检查新增部分（→ Skills 二-6） | 轻微 |
...
2. 台词保留与密度控制——是否默认逐字保留原文台词；是否只把完全重复或功能性低密度整段列为可删候选，未擅自补写台词
