...
5. **像素为锚** — 所有变体必须保持像素矢量插画质感，拒绝 3D 渲染/CG 动画感；保持像素轮廓、有限色填充
...
| 全景 → 中景 | 材质、色调、光线 | 裁切聚焦、有限色变化 |
| 中景 → 近景 | 材质、色调 | 有限色聚焦、背景色块 |
| 近景 → 特写 | 色块纹理 | 有限色聚焦、微距色块 |
...
| 时段 | 视觉特征 | 提示词 |
|---|---|---|
| 清晨 | 像素色调、浅色色块 | 像素清晨、浅色晨色 |
| 正午 | 像素明亮、有限色色块 | 像素正午、有限色明亮 |
| 黄昏 | 像素金色、暖色色块 | 像素黄昏、暖色金辉 |
| 夜间（月光） | 像素冷蓝、深色色块 | 像素月光、冷蓝月色 |
| 夜间（灯火） | 像素暖黄、深色背景 | 像素灯火、暖黄暗底 |
...
| 天候 | 视觉特征 | 提示词 |
|---|---|---|
| 晴天 | 像素明亮、有限色色块 | 像素晴天、有限色晴日 |
| 阴天 | 像素均匀、灰色色块 | 像素阴天、灰色柔光 |
| 薄雾 | 像素朦胧、低饱和色块 | 像素薄雾、朦胧色块 |
| 细雨 | 像素雨丝、湿润色块 | 像素细雨、湿润色块 |
| 飞雪 | 像素白色、覆盖色块 | 像素飞雪、白色覆盖 |

### 天候衍生规范

| 从基准天候衍生 | 保持不变 | 变化项 |
|---|---|---|
| 晴 → 薄雾 | 建筑/布局 | 增加像素雾层、远景色块模糊、饱和度降低 |
| 晴 → 细雨 | 建筑/布局 | 增加像素雨丝、地面色块、色调偏冷 |
| 晴 → 飞雪 | 建筑/布局 | 增加像素积雪、雪花色块、色调偏白 |
| 植被需随天候逻辑适配 | — | 像素雨色、像素雪色 |
...
| R8 | 必须指定「像素风格」关键词（2d flat design、vector art） |
| R9 | 必须指定「无光影无渐变」 |
| R10 | 材质必须为有限色填充，禁止复杂纹理/岁月感 |
