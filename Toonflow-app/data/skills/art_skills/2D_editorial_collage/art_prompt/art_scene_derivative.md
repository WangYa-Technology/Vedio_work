...
5. **拼贴为锚** — 所有变体必须保持拼贴矢量插画质感，拒绝 3D 渲染/CG 动画感；保持剪纸边缘、印刷色块填充
...
| 全景 → 中景 | 材质、色调、光线 | 裁切聚焦、印刷色块变化 |
| 中景 → 近景 | 材质、色调 | 印刷色块聚焦、背景色块 |
| 近景 → 特写 | 色块纹理 | 印刷色块聚焦、微距色块 |
...
| 时段 | 视觉特征 | 提示词 |
|---|---|---|
| 清晨 | 拼贴色调、浅色色块 | 拼贴清晨、浅色晨色 |
| 正午 | 拼贴明亮、印刷色块色块 | 拼贴正午、印刷色块明亮 |
| 黄昏 | 拼贴金色、暖色色块 | 拼贴黄昏、暖色金辉 |
| 夜间（月光） | 拼贴冷蓝、深色色块 | 拼贴月光、冷蓝月色 |
| 夜间（灯火） | 拼贴暖黄、深色背景 | 拼贴灯火、暖黄暗底 |
...
| 天候 | 视觉特征 | 提示词 |
|---|---|---|
| 晴天 | 拼贴明亮、印刷色块色块 | 拼贴晴天、印刷色块晴日 |
| 阴天 | 拼贴均匀、灰色色块 | 拼贴阴天、灰色柔光 |
| 薄雾 | 拼贴朦胧、低饱和色块 | 拼贴薄雾、朦胧色块 |
| 细雨 | 拼贴雨丝、湿润色块 | 拼贴细雨、湿润色块 |
| 飞雪 | 拼贴白色、覆盖色块 | 拼贴飞雪、白色覆盖 |

### 天候衍生规范

| 从基准天候衍生 | 保持不变 | 变化项 |
|---|---|---|
| 晴 → 薄雾 | 建筑/布局 | 增加拼贴雾层、远景色块模糊、饱和度降低 |
| 晴 → 细雨 | 建筑/布局 | 增加拼贴雨丝、地面色块、色调偏冷 |
| 晴 → 飞雪 | 建筑/布局 | 增加拼贴积雪、雪花色块、色调偏白 |
| 植被需随天候逻辑适配 | — | 拼贴雨色、拼贴雪色 |
...
| R8 | 必须指定「拼贴风格」关键词（2d flat design、vector art） |
| R9 | 必须指定「无光影无渐变」 |
| R10 | 材质必须为印刷色块填充，禁止复杂纹理/岁月感 |
