# 视频提示词生成 （通用首尾帧模式，历史参考）

> 当前生产链路使用 `src/routes/production/workbench/generateVideoPrompt.ts` 和数据库中的视频推理模板；本文件不会被运行时自动加载。修改本文件不会直接改变工作台行为，新的模型规则应先进入运行时契约与模板版本记录。
