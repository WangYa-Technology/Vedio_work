...
export default router.post(
  ...
  async (req, res) => {
    const { projectId, assetsId, templateId, referenceImageId } = req.body;
    const asset = await u
      .db("o_assets")
      ...
      .first();
    if (!asset)
      return res.status(404).send(error("资产不存在或不属于当前项目"));
    ...
    const template = await u
      ...
      .first();
    const templateSource = String(template?.useData || template?.data || "").trim();
    ...
    try {
      ...
      const structuredTemplate = parseAssetInferenceTemplate(templateSource);
      if (structuredTemplate && !isAssetInferenceTemplateApplicable(structuredTemplate, asset.type as "role" | "scene" | "tool")) {
      ...
      const renderedTemplate = structuredTemplate
        ...
        : renderTemplate(templateSource, asset);
      ...
      if (selectedReferenceId) {
        const image = await u.db("o_image").where({ id: selectedReferenceId }).select("filePath").first();
        if (image?.filePath) {
          try {
      ...
      const factsMessage = {
        ...
        content: [
          ...
          ...(referenceImageBytes ? [{ type: "image" as const, image: referenceImageBytes }] : []),
        ],
      };
      let result: any;
      try {
      ...
      if (!prompt)
        return res.status(502).send(error("图片推理未返回有效提示词"));
      ...
      await u
        .db("o_assets")
        .where({ id: assetsId, projectId })
        .update({ originalPrompt, prompt, promptState: "已完成", promptErrorReason: null });
      return res
        .status(200)
        .send(success({ originalPrompt, prompt, assetsId, templateId }));
    } catch (caught) {
      await u
        .db("o_assets")
        .where({ id: assetsId, projectId })
        .update({ promptState: "失败", promptErrorReason: u.error(caught).message });
      return res
        .status(502)
        .send(error(`图片提示词推理失败：${u.error(caught).message}`));
    }
  },
);
