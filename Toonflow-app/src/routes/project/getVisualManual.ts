...
const DATA_MAP: { label: string; value: string; subDir?: string }[] = [
  ...
  { label: "提示词推理模板", value: "prompt_reasoning", subDir: "art_prompt" },
  {
    label: "技法-导演规划",
    value: "director_planning_style",
    subDir: "driector_skills",
  },
  {
    label: "技法-分镜表设计",
...
async function readAllImages(imagesDir: string) {
  try {
    const ossPath = u.getPath(
      path.join("skills", "art_skills", imagesDir, "images"),
    );
    ...
    if (images.length) {
      return Promise.all(
        images.map(async (i) => await u.oss.getFileUrl(i, "skills")),
      );
    } else {
      return [];
    }
