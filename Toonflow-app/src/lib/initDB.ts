...
function extractVendorMeta(code?: string | null): Partial<BundledVendor> {
  if (!code) return {};
  return {
    version: readCodeString(code.match(/version:\s*"((?:\\.|[^"\\])*)"/s)?.[1]),
...
function compareVersions(left?: string, right?: string) {
  const normalize = (value?: string) => String(value || "").split(".").map((part) => Number(part) || 0);
...
export default async (knex: Knex, forceInit: boolean = false): Promise<void> => {
  const tables: TableSchema[] = [
        ...
        await knex("o_prompt").insert([
          {
            ...
            name: "忠实分镜型",
            type: "videoPromptGeneration",
            data: DEFAULT_VIDEO_PROMPT_FAITHFUL,
            useData: DEFAULT_VIDEO_PROMPT_FAITHFUL,
          },
          {
            id: 1786742322711,
            name: "动作简化型",
            type: "videoPromptGeneration",
            data: DEFAULT_VIDEO_PROMPT_ACTION,
            useData: DEFAULT_VIDEO_PROMPT_ACTION,
          },
        ]);
      },
    },
    ...
    //视频
    {
      ...
      builder: (table) => {
        ...
        table.text("promptSnapshot");
        table.integer("promptTemplateId");
        table.text("promptTemplateVersion");
        table.text("referenceSnapshot");
        table.text("generationConfig");
        table.text("outputAudit");
        table.integer("createTime");
        table.primary(["id"]);
        table.unique(["id"]);
      },
    ...
    // 视频轨道
    {
      ...
      builder: (table) => {
        ...
        table.integer("promptTemplateId");
        table.text("promptTemplateVersion");
        table.text("promptInferenceSnapshot");
  ...
  if ((await knex.schema.hasTable("o_project")) && !(await knex.schema.hasColumn("o_project", "negativePrompt"))) {
    ...
    console.log("[初始化数据库] 已补充项目负向提示词字段: negativePrompt");
  }
  if (await knex.schema.hasColumn("o_project", "negativePrompt")) {
  ...
  if ((await knex.schema.hasTable("o_assets")) && !(await knex.schema.hasColumn("o_assets", "originalPrompt"))) {
    ...
    console.log("[初始化数据库] 已补充资产原始提示词字段: originalPrompt");
  }
  if (await knex.schema.hasColumn("o_assets", "originalPrompt")) {
    await knex("o_assets")
  ...
  if (await knex.schema.hasTable("o_assets2Storyboard")) {
    const removedOrphanBindings = await knex("o_assets2Storyboard")
      .whereNotExists(function () {
        this.select(1)
          .from("o_storyboard")
          .whereRaw("o_storyboard.id = o_assets2Storyboard.storyboardId");
      })
      .del();
    if (removedOrphanBindings) {
      console.log(`[初始化数据库] 已清理 ${removedOrphanBindings} 条孤立分镜资产关联`);
    }
  }
  ...
  if (videoTableExists) {
    const videoColumns: Array<{ name: string; type: "text" | "integer" }> = [
      ...
      { name: "promptSnapshot", type: "text" },
      { name: "promptTemplateId", type: "integer" },
      { name: "promptTemplateVersion", type: "text" },
      { name: "referenceSnapshot", type: "text" },
      { name: "generationConfig", type: "text" },
      { name: "outputAudit", type: "text" },
      { name: "createTime", type: "integer" },
    ];
  ...
  const videoTrackTableExists = await knex.schema.hasTable("o_videoTrack");
  if (videoTrackTableExists) {
    const videoTrackColumns: Array<{ name: string; type: "text" | "integer" }> = [
      { name: "promptTemplateId", type: "integer" },
      { name: "promptTemplateVersion", type: "text" },
      { name: "promptInferenceSnapshot", type: "text" },
    ];
    for (const column of videoTrackColumns) {
      if (await knex.schema.hasColumn("o_videoTrack", column.name)) continue;
      await knex.schema.alterTable("o_videoTrack", (table) => {
        column.type === "integer" ? table.integer(column.name) : table.text(column.name);
      });
      console.log("[初始化数据库] 已补充分镜提示词字段:", column.name);
    }
  }
};
