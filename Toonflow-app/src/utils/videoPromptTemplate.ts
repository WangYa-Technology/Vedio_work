...
2. 以 segment.videoDescription 为内容依据。该字段已按镜头顺序包含画面与动作、时长、景别、运镜、台词和音效，必须完整使用。
3. 台词不可遗漏、改写或翻译。普通对白、内心独白 OS、画外音 VO 必须保留原类型；无台词时明确写“无台词”。
4. 音效不得替代台词；镜头动作、运镜、景别与时长不得相互混淆。
5. 只允许使用当前 segment.referenceMap 中列出的文字资产映射。输入不包含图片；编号按列表顺序从 @图1 开始，输出必须逐行保留全部映射，不得改变编号或资产名称，禁止引用故事板图、分镜图或不存在的编号。
...
[视频内容]
按时间顺序描述该片段内全部镜头。明确人物动作、表情、场景、景别、运镜、台词与音效；保持镜头衔接自然，总时长与输入一致且不超过 15 秒。
